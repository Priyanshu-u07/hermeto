...
def _serialize_value(value: Any, *, raw: bool = False) -> Any:
    ...
    if isinstance(value, dict):
        return {k: _serialize_value(v, raw=raw) for k, v in value.items()}
    if isinstance(value, list):
        return [_serialize_value(v, raw=raw) for v in value]
...
def _get_section_dict(section_obj: Any, *, raw: bool = False) -> dict[str, Any]:
    """Extract and serialize all fields from a config section object."""
    return {
        field_name: _serialize_value(getattr(section_obj, field_name), raw=raw)
...
def get_effective_config(config: Config, *, raw: bool = False) -> dict[str, Any]:
    ...
    for name in Config.model_fields:
        ...
        if _is_section(value):
            result[name] = _get_section_dict(value, raw=raw)
        else:
            result[name] = _serialize_value(value, raw=raw)
    return result
...
def _format_yaml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        dumped = yaml.dump(value, default_flow_style=True, width=float("inf")).strip()
